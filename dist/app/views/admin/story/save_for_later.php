    ...
    if ($value['desc'] != '' || $key == (sizeof($data['images']) - 1)) {
        if ($row == 1) {
            ...
            if ($size[$row] == 4) {
                if ($size[$row - 1] == 12) {
                    ...
                    $content[$row - 1] .= $content[$row];
                    // $content[$row - 1] = str_replace('col-md-6', 'col-md-8', $content[$row - 1]);
                }
            } else if ($size[$row] == 8) {
                if ($size[$row - 1] == 8) {
                    $content[$row - 1] .= $content[$row];
        ...
        if ($key != (sizeof($data['images']) - 1)) {
            ...
            if ($value['desc_position'] == 1) {
                $content[$row] .= '<div class="col-12 text-justify align-self-center"><div class="py-3">' . $value['desc'] . '</div></div>';
            } else if ($value['desc_position'] == 4) {
                $content[$row] .= '<div class="col text-justify align-self-center"><div class="py-3">' . $value['desc'] . '</div></div>';
            }
            if ($value['orientation'] == 0) {
                $content[$row] .= '<div class="align-self-center ' . ($value['desc_position'] == 2 || $value['desc_position'] == 4 ? 'col-md-4' : 'col-12') . '"><div class="position-relative"><div class="ratio ratio-portrait"><img src="' . IMGURL . $value['picture'] . '" class="object-fit-cover shadow-sm"></div>' . ($value['is_best'] == 1 ? '<div class="position-absolute top-0 end-0"><div class="px-2 py-1 m-2 bg-success rounded-1 shadow" title="Best"><i class="bi bi-hand-thumbs-up text-white"></i></div></div>' : '') . '</div></div>';
            } else {
                $content[$row] .= '<div class="align-self-center ' . ($value['desc_position'] == 2 || $value['desc_position'] == 4 ? 'col-md-6' : 'col-12') . '"><div class="position-relative"><div class="ratio ' . ($value['desc_position'] == 2 || $value['desc_position'] == 4 ? 'ratio-portrait' : 'ratio-16x9') . '">    <img src="' . IMGURL . $value['picture'] . '" class="object-fit-cover shadow-sm"></div>' . ($value['is_best'] == 1 ? '<div class="position-absolute top-0 end-0"><div class="px-2 py-1 m-2 bg-success rounded-1 shadow" title="Best"><i class="bi bi-hand-thumbs-up text-white"></i></div></div>' : '') . '</div></div>';
            }
        ...
        if ($value['orientation'] == 0) {
            $size[$row] += 4;
            $currentSize = 4;
            $content[$row] .= '<div class="col-md-4"><div class="position-relative"><div class="ratio ratio-portrait"><img src="' . IMGURL . $value['picture'] . '" class="object-fit-cover shadow-sm"></div>' . ($value['is_best'] == 1 ? '<div class="position-absolute top-0 end-0"><div class="px-2 py-1 m-2 bg-success rounded-1 shadow" title="Best"><i class="bi bi-hand-thumbs-up text-white"></i></div></div>' : '') . '</div></div>';
        } else {
            $size[$row] += 8;
            $currentSize = 8;
            $content[$row] .= '<div class="col-md-8"><div class="position-relative"><div class="ratio ratio-landscape"><img src="' . IMGURL . $value['picture'] . '" class="object-fit-cover shadow-sm"></div>' . ($value['is_best'] == 1 ? '<div class="position-absolute top-0 end-0"><div class="px-2 py-1 m-2 bg-success rounded-1 shadow" title="Best"><i class="bi bi-hand-thumbs-up text-white"></i></div></div>' : '') . '</div></div>';
        }
    }
